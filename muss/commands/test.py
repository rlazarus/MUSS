...
class FooOne(parser.Command):
    ...
    @classmethod
    def args(cls, player):
        return pyparsing.Word(pyparsing.alphas)
...
class FooTwo(parser.Command):
    ...
    @classmethod
    def args(cls, player):
        return pyparsing.Word(pyparsing.alphas) + pyparsing.Optional(pyparsing.Word(pyparsing.alphas))
...
class FooThree(parser.Command):
    ...
    @classmethod
    def args(cls, player):
        return pyparsing.Word(pyparsing.alphas) * 3 + pyparsing.Optional(pyparsing.Word(pyparsing.alphas) + pyparsing.Word(pyparsing.alphas))
...
class Ptest(parser.Command):
    ...
    def execute(self, player, args):
        ...
        player.enter_mode(handler.PromptMode(player,"Enter text", handle_response))
