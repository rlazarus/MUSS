...
class SomeOfTestCase(parser_tools.ParserTestCase):
    def test_easy(self):
        result = object()
        token = parser.SomeOf([("foo", result), ("bar", object())])
        self.assert_parse(token, "foo", [result])

    def test_partial(self):
        result = object()
        token = parser.SomeOf([("foo", result), ("bar", object())], exact=False)
        self.assert_parse(token, "f", [result])

    def test_partial_ambiguous(self):
        bar_result = object()
        baz_result = object()
        token = parser.SomeOf([("bar", bar_result), ("baz", baz_result)],
                              exact=False)
    ...
    def test_full_ambiguous(self):
        result1 = object()
        result2 = object()
        token = parser.SomeOf([("foo", result1), ("foo", result2)], exact=False)
        self.assert_parse(token, "foo", [result1, result2])


    def test_prefer_full(self):
        result = object()
        token = parser.SomeOf([("foo", result), ("foobar", object())],
                             exact=False)
        self.assert_parse(token, "foo", [result])
    ...
    def test_partial_but_exact(self):
        token = parser.SomeOf([("foo", object()), ("bar", object())],
                             exact=True)
    ...
    def test_notfound(self):
        self.assertRaises(parser.NotFoundError,
                          parser.SomeOf([("foo", object())]).parseString, "bar",
                                        parseAll=True)
